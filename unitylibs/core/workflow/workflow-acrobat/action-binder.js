...
export default class ActionBinder {
  ...
  async handleSingleFileUpload(file, eventName) {
    ...
    if (!accountType) {
      await this.dispatchErrorToast('verb_upload_error_generic', 500, 'Account type not found', false);
      return;
    }
  ...
  async getLimits(verb) {
    try {
      this.limits = await this.loadVerbLimits(this.workflowCfg.name, ActionBinder.LIMITS_MAP[verb]);
    } catch (e) {
      await this.dispatchErrorToast('verb_upload_error_generic', 500, 'Error loading verb limits', false);
    }
  }

  async processSingleFile(files, eventName) {
    await this.getLimits(this.workflowCfg.enabledFeatures[0]);
