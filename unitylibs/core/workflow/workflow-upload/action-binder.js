...
class ServiceHandler {
  ...
  showErrorToast(errorCallbackOptions, error, lanaOptions) {
    ...
    window.lana?.log(msg, errorUpdate, lanaOptions);
  }
}
...
export default class ActionBinder {
  constructor(unityEl, workflowCfg, wfblock, canvasArea, actionMap = {}) {
    ...
    this.lanaOptions = { tags: 'Project Unity (PS-Upload)' };
  }
  ...
  async cancelUploadOperation() {
    try {
      ...
      await this.transitionScreen?.showSplashScreen();
      window.lana?.log('Error cancelling upload operation', error, this.lanaOptions);
      throw error;
    }
  }
  ...
  async uploadImgToUnity(storageUrl, id, blobData, fileType) {
    ...
    if (response.status !== 200) {
      window.lana?.log('Failed to upload image to Unity', response.status, this.lanaOptions);
  ...
  async createErrorToast() {
    try {
      ...
      window.lana?.log('Error creating error toast', e, this.lanaOptions);
      return null;
    }
  }
