...
export default class TransitionScreen {
  ...
  createProgressBar() {
    const pdom = `<div class="spectrum-ProgressBar spectrum-ProgressBar--sizeM spectrum-ProgressBar--sideLabel" value="0" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="${this.LOADER_LIMIT}">
  ...
  progressBarHandler(s, delay, i, initialize = false) {
    if (!s) return;
    const nextDelay = Math.min(delay + 100, 2000);
    const nextI = Math.max(i - 5, 5);
    ...
    setTimeout(() => {
      ...
      if (v === 100) return;
      this.updateProgressBar(s, v + nextI);
      this.progressBarHandler(s, nextDelay, nextI);
    }, nextDelay);
  }
