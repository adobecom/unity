...
if [[ "$GITHUB_BASE_REF" == "main" ]]; then
  # extract base branch name from GITHUB_BASE_REF
  target_branch="main"
  echo "Target branch: $target_branch"
fi

# TODO ADD HLX5 SUPPORT
PR_BRANCH_LIVE_URL_GH="https://$target_branch--dc--$prOrg.aem.live"
...
npx playwright test --config=./playwright.config.cjs ${TAGS} ${EXCLUDE_TAGS} --project=unity-live-chromium --project=unity-live-firefox --project=unity-live-webkit ${REPORTER} || EXIT_STATUS=$?
