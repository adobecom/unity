...
import { features } from './ai-chat-pdf.spec.cjs';
import AiChatPdf from './ai-chat-pdf.page.cjs';
...
test.describe('Unity AI Chat PDF test suite', () => {
  ...
  test(`${features[0].name},${features[0].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-1: Go to AI Chat PDF test page', async () => {
      await page.goto(`${baseURL}${features[0].path}${unityLibs}`);
      await page.waitForLoadState('domcontentloaded');
      await expect(page).toHaveURL(`${baseURL}${features[0].path}${unityLibs}`);
    });

    await test.step('step-2: Verify AI Chat PDF content/specs', async () => {
      await expect(await aiChatPdf.widget).toBeVisible();
      await expect(await aiChatPdf.dropZone).toBeVisible();
      await expect(await aiChatPdf.verbImage).toBeVisible();
      await expect(await aiChatPdf.acrobatIcon).toBeVisible();
