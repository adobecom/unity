  ...
  test(`${features[0].name},${features[0].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-3: Upload a sample PDF file', async () => {
      ...
      if (xApiClientId === 'unity') {
        ...
        const expectedPath = '/drafts/nala/acrobat/online/test/compress-pdf';
        ...
        throw new Error(`Incomplete URL parameters. Expected x_api_client_id=unity or null, but got: ${xApiClientId}. Expected x_api_client_location=compress-pdf or null, but got: ${xApiClientLocation}`);
      }
